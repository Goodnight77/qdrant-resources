...
def main():
    ...
    mem_mb = {k: n_base * DIM * 4 / 1e6 / COMPRESS[k] for k in R}  # resident vector bytes
    names = list(R.keys())
    ...
    fig.suptitle(f"Qdrant Quantization Face-Off — dbpedia 1536-d, {n_base:,} vectors", fontsize=14, weight="bold")
    ...
    for i, n in enumerate(names):
        ax[0, 0].text(i, R[n]["recall_at_k"] + 0.01, f"{R[n]['recall_at_k']:.3f}", ha="center")

    # latency percentiles
    x = range(len(names))
    w = 0.25
    for j, (p, c) in enumerate([("p50", "#8172B3"), ("p95", "#CCB974"), ("p99", "#C44E52")]):
    ...
    ax[1, 0].set_ylim(0, max(mem_mb.values()) * 1.25)  # headroom so labels clear the title
    for i, n in enumerate(names):
        ax[1, 0].text(i, mem_mb[n] + max(mem_mb.values()) * 0.02,
                      f"{mem_mb[n]:.0f} MB ({COMPRESS[n]}x)", ha="center")

    # tradeoff: recall vs p50 latency, bubble size = memory
    for i, n in enumerate(names):
        ax[1, 1].scatter(R[n]["latency_p50_ms"], R[n]["recall_at_k"],
                         s=mem_mb[n] * 2, color=colors[i], alpha=0.7, label=labels[i])
    ...
    print("wrote", path)
    for n in names:
        print(f"{LABEL.get(n, n):8} recall={R[n]['recall_at_k']:.3f}  p50={R[n]['latency_p50_ms']:.1f}ms  ram={mem_mb[n]:.0f}MB ({COMPRESS[n]}x)")
